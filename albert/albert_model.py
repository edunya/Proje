...
def tokenize_function(examples):
    return tokenizer(examples["text"], padding="max_length", truncation=True, max_length=128)  # Max length eklendi
...
if os.path.exists("./albert_model"):
    ...
    model = AlbertForSequenceClassification.from_pretrained(
        "albert-base-v2", num_labels=len(label_encoder.classes_)
    )
...
training_args = TrainingArguments(
    ...
    save_strategy="epoch",
    logging_dir="./logs",
    learning_rate=3e-6,  # Daha hızlı eğitim için öğrenme oranı artırıldı
    weight_decay=0.01,
    per_device_train_batch_size=16,  # Daha küçük batch size
    per_device_eval_batch_size=16,
    num_train_epochs=6,  # Daha az epoch
    save_total_limit=1,
...
if not os.path.exists("./albert_model"):
